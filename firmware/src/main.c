...
typedef union _INTPUT_CONTROLS_TYPEDEF
{
    ...
    {
        BYTE buttons_1_to_8;
        BYTE button_9_and_padding;
        BYTE hat_and_padding;
    } members;
    BYTE val[3];
} INPUT_CONTROLS;
...
BYTE bootFeatureReport[SWANSONG_ENTER_REPORT_SIZE] __at(0x2060);
    ...
    //accessible portion of microcontroller RAM.
        ...
        #define HID_REPORT_ADDRESS (INPUT_CONTROLS_ADDRESS + HID_INT_IN_EP_SIZE)
        #define INPUT_CONTROLS_ADDRESS_TAG __at(INPUT_CONTROLS_ADDRESS)
        #define HID_REPORT_ADDRESS_TAG __at(HID_REPORT_ADDRESS)
    #else
...
BYTE hid_report[3] HID_REPORT_ADDRESS_TAG;
...
//        }
    #endif
#elif defined(_PIC14E) && defined(USB_INTERRUPT)
    	//These are your actual interrupt handling routines.
...
void UserInit(void)
{
    ...
    /* All membrane contacts are active-low. Ports A/B use the PIC's weak
       pull-ups; Port C is held high by RN1/RN2 on the production PCB. */
    TRISA |= 0x10;
    TRISB |= 0xF0;
    TRISC |= 0xFF;
    WPUA |= 0x10;
...
void Joystick(void)
{
    ...
    BYTE hat = HAT_SWITCH_NULL;

    /* Full-speed USB supplies one SOF every millisecond. Sampling only once
       per SOF makes the five-count integrator a real 5 ms debounce filter. */
    if(sofTick == 0)
    {
        return;
    }
    ...
    rawPressed[0]  = !BTN_X1;     /* D-pad up */
    rawPressed[1]  = !BTN_X2;     /* D-pad right */
    rawPressed[2]  = !BTN_X3;     /* D-pad down */
    rawPressed[3]  = !BTN_X4;     /* D-pad left */
    rawPressed[4]  = !BTN_A;
    rawPressed[5]  = !BTN_B;
    rawPressed[6]  = !BTN_Y1;
    rawPressed[7]  = !BTN_Y2;
    rawPressed[8]  = !BTN_Y3;
    rawPressed[9]  = !BTN_Y4;
    rawPressed[10] = !BTN_START;
    rawPressed[11] = !BTN_SOUND;
    rawPressed[12] = !BTN_POWER;

    for(i = 0; i < 13; i++)
    {
        if(rawPressed[i])
        {
            if(debounceCount[i] < 5)
            {
                debounceCount[i]++;
            }
            if(debounceCount[i] == 5)
            {
                stableButtons |= ((WORD)1 << i);
            }
        }
        else
        {
    ...
    /* Nine ordinary gamepad buttons: A, B, Y1-Y4, Start, Sound, Power. */
    joystick_input.val[0] = (BYTE)((stableButtons >> 4) & 0xFF);
    joystick_input.val[1] = (BYTE)((stableButtons >> 12) & 0x01);

    up    = (stableButtons & ((WORD)1 << 0)) != 0;
    right = (stableButtons & ((WORD)1 << 1)) != 0;
    down  = (stableButtons & ((WORD)1 << 2)) != 0;
    left  = (stableButtons & ((WORD)1 << 3)) != 0;

    if(up && !down)
    {
        if(right && !left) hat = HAT_SWITCH_NORTH_EAST;
        else if(left && !right) hat = HAT_SWITCH_NORTH_WEST;
        else hat = HAT_SWITCH_NORTH;
    }
    else if(down && !up)
    {
        if(right && !left) hat = HAT_SWITCH_SOUTH_EAST;
        else if(left && !right) hat = HAT_SWITCH_SOUTH_WEST;
        else hat = HAT_SWITCH_SOUTH;
    }
    else if(right && !left)
    {
        hat = HAT_SWITCH_EAST;
